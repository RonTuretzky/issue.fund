...
interface DeploymentVm { function startBroadcast() external; function stopBroadcast() external; function envAddress(string calldata) external returns (address); }
contract Deploy {
    ...
    function run() external returns (GithubDkimVerifier verifier, MergeBounty escrow) {
        ...
        escrow = new MergeBounty(verifier, vm.envAddress("FEE_RECIPIENT"), 100);
        vm.stopBroadcast();
    }
}
