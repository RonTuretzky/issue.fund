...
| Escrow (new bounties)      | `0x1f5cE96dFa05D207Ca8E59C6ab4B9F1895D24630` |
| Historical no-fee escrow         | `0xf7d518780fb08d77a79efdc9fe8f9fd32bdd6d46` |
| Initial fee recipient / owner | `0x86213f1cf0a501857B70Df35c1cb3C2EcF112844` |
| Success fee                | 1% (100 basis points), fixed                 |
| Chain                         | Gnosis, 100                                  |
| Currency                      | Native xDAI                                  |

Current deployment: [confirmed transaction](https://gnosisscan.io/tx/0xb0dc07f9a2d723caa78e80c0968c652c0771f9d6890a17ded170ef5cdb5e0caf), September 10, 2026. The initial owner can change the fee recipient; ownership transfers require acceptance. The deployer has no fee administration rights.

Original deployment transactions: [verifier](https://gnosisscan.io/tx/0x2c8b9f6872626f92ceb0880445c25ef58eef5ea66631214a5424806550ed9e54), [escrow](https://gnosisscan.io/tx/0xacd7879b9e2c12626be43832fda88cc6d2a703c45c6c6998cab86588805a997f).
...
The manual workflow in `.github/workflows/deploy-gnosis.yml` uses BreadchainCoop/etherform pinned to `180aa91926a02ce2045f9a7e3ed08be9395ecfab`. It invokes `script/Deploy.s.sol:Deploy`, which requires chain 100. Configure `FEE_RECIPIENT` in the deployment environment plus the repository's Gnosis secret variables before dispatch; creating the workflow does not provision those secrets.

For local operation, `forge build` followed by `npm run deploy:gnosis:plan` prepares a read-only deployment plan. `node scripts/deploy-gnosis.mjs --execute` uses the reviewed terms and preserves earlier escrows; it accepts a key through hidden terminal input or `GNOSIS_DEPLOYER_KEY` supplied by a secret manager. Never pass the key as a CLI argument or commit it. The script records public transaction checkpoints, resumes interrupted deployment and validates the resulting manifest.
...
Both browser tests passed against `https://issue.fund`. Checks covered local receipt validation before disclosure consent, canonical bounty links, paid/fee events, beneficiary and treasury balances, and replay rejection with `NotOpen`. A read-only treasury withdrawal simulation succeeded; no treasury withdrawal or owner action was signed. Public evidence is in [v2-e2e.json](deployments/gnosis/v2-e2e.json). Raw originals and signing secrets remain excluded from Git.
...
On September 10, 2026, [issue #7](https://github.com/RonTuretzky/tmp-mergebounty-public-e2e-20260906/issues/7) and [PR #8](https://github.com/RonTuretzky/tmp-mergebounty-public-e2e-20260906/pull/8) completed the **server-submitted** flow on [Bounty #2](https://issue.fund/#bounty/100/0x1f5ce96dfa05d207ca8e59c6ab4b9f1895d24630/2).
...
## Historical no-fee GitHub → Gnosis acceptance test
...
There is one maintained `MergeBounty` implementation. Keep historical escrows and their credits accessible. The deployment
...
2. Set public environment values `FEE_RECIPIENT`, `EXPECTED_DEPLOYER`, and
   optionally `CLAIM_FEE_BPS` (default `100`). Run `npm run deploy:gnosis:plan`.
...
3. Review that plan. Run `node scripts/deploy-gnosis.mjs --execute`, supplying
...
6. Run `node scripts/promote-manifest.mjs` to revalidate and promote the
...
7. Complete the real-mail acceptance checks and enable the service configuration
   deliberately; funding a bounty does not by itself enable automatic receipt
...
Historical `v2-` checkpoint filenames are intentionally retained so an interrupted transaction can be reconciled without accidental rebroadcast. New manifests record `sourceContract: "MergeBounty"`; historical source/ABI records are under `deployments/archive/2026-09-10`. Source cleanup does not move funds or require deploying identical executable logic.
