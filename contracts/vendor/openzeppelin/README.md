...
This branch is a review candidate. It has not been deployed or promoted to the live app.
...
Verified on 2026-09-14: 59 Solidity tests and 25 JavaScript tests passed. Fresh local deployments completed create, permissionless claim, fee credit, replay rejection and withdrawal for both 1024-bit and 2048-bit synthetic keys. The candidate also verified both saved original GitHub merge/closure notifications on an isolated local chain; raw emails were not transmitted to a public RPC. These are local results, not a live Gnosis deployment or independent audit. The vendored baseline was byte-compared with the upstream GitHub source.

## Deployment decision

A fresh verifier and escrow are required if this candidate is adopted. No compatibility migration is required by the user. The existing public deployment is unchanged while the explicitly unaudited key-size exception is under review. Switching this primitive does not independently audit our GitHub receipt parsing or bounty settlement policy, and this branch does not substitute a different escrow protocol.
